# ==============================================================================
# Title:        GWR Prediction by Time Points
# Description:  Runs GWR predictions for each time point using time-specific
#               optimal bandwidths from previous calibration results.
...
CONFIG <- list(
  # --- File Paths ---
  calibration_data_path = "./calib_diffdisp/20251016_GTWR_InputData_DiffDisp_Layer_All.csv",
  # prediction_data_path = "grid_pnt.csv",
  prediction_data_path = "diffdisp_gridpnt.feather",
  
  # Path to the summary CSV file containing optimal bandwidths for each time point
  bandwidth_summary_path = "./GWR_Output_Layer_All/GWR_Layer_All_bisquare_summary.csv",
  ...
  # --- Column Selection ---
  columns_to_preserve = c("monthly", "datetime_str", "X_TWD97", "Y_TWD97", "DIFFDISP"),  # Add columns you want to keep
...
suppressPackageStartupMessages({
  library("GWmodel")
  library("sp")
  library("arrow")  # Add this line - arrow provides read_feather()
})
...
# Load bandwidth summary
if (!file.exists(CONFIG$bandwidth_summary_path)) {
  stop("Bandwidth summary file not found: ", CONFIG$bandwidth_summary_path)
}
bandwidth_summary <- read.csv(CONFIG$bandwidth_summary_path)
cat("   - Loaded bandwidth summary with", nrow(bandwidth_summary), "time points\n")
...
for (i in seq_along(selected_time_points)) {
  ...
  # Get bandwidth for this time point
  bandwidth_row <- bandwidth_summary[bandwidth_summary$time_point == current_time, ]
  if (nrow(bandwidth_row) == 0) {
    cat("   WARNING: No bandwidth found for time", current_time, ". Skipping.\n")
    next
  }
  
  optimal_bw <- bandwidth_row$optimal_bandwidth[1]
  cat("   - Using bandwidth:", optimal_bw, "\n")
  ...
  tryCatch({
    gwr_predictions <- gwr.predict(
      ...
      adaptive = CONFIG$use_adaptive_bandwidth,
    ...
    if (CONFIG$save_individual_predictions) {
      ...
      cat("   - Prediction saved to:", prediction_filename, "\n")
...
if (nrow(prediction_summary) > 0) {
  ...
  if (CONFIG$save_summary_table) {
    summary_filename <- sprintf("GWR_prediction_%s_%s_summary.csv",
