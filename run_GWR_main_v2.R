...
CONFIG <- list(
  # --- File Paths ---
  input_csv_path = "./calib_diffdisp/20251016_GTWR_InputData_DiffDisp_Layer_All.csv",
  output_directory = "GWR_Output_Layer_All",
  
  # --- Model Parameters ---
  formula_string = "Layer_All ~ CUMDISP",
  ...
  preserve_columns = c("STATION", "Layer_All", "CUMDISP"),
...
if (nrow(results_summary) > 0) {
  ...
  cat("Average bandwidth:", round(mean(results_summary$optimal_bandwidth, na.rm = TRUE), 2), "\n")
  cat("Bandwidth range:", round(min(results_summary$optimal_bandwidth, na.rm = TRUE), 2), 
      "to", round(max(results_summary$optimal_bandwidth, na.rm = TRUE), 2), "\n")
  
  if (!all(is.na(results_summary$R_squared))) {
    cat("Average R-squared:", round(mean(results_summary$R_squared, na.rm = TRUE), 3), "\n")
  }
